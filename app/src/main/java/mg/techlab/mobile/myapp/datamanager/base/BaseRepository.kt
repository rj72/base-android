package mg.techlab.mobile.myapp.datamanager.base
...
import mg.techlab.mobile.myapp.Closure
...
open class BaseRepository<T : RealmModel>(val ofType: Class<T>) {
    ...
    fun insert(
        ...
        try {
            ...
            ignored.localizedMessage?.let { Log.e("EXCEPTION",it) }
    ...
    fun deleteAll(completion: Closure<Boolean>? = null) {
        try {
            realm = Realm.getDefaultInstance()
            realm?.executeTransaction {
